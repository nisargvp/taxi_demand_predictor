...
def fetch_features_and_targets_from_store(
    ...
    """
    Fetched time-series data from the feature store, transforms it into features and 
    target and returns it as a pandas DataFrame.
    """
    # get pointer to the feature view
    logger.info('Getting pointer to the feature view...')
    feature_view = get_or_create_feature_view(FEATURE_VIEW_METADATA)
    
    # generate training data from the feature view
    logger.info('Generating training data from the feature view...')
    ...
    # sorrt by pickup_location_id and pickup_hour in ascending order
...
def train(
    ...
    logger.info(f'Splitting data into training and validation sets with {cutoff_date}')
    ...
    # push the model to the Comet ML model registry if it meets the minimum performance threshold
    ...
    if test_mae < config.MAX_MAE:
        ...
        logger.info(f'Model did not meet the minimum performance threshold of {config.MAX_MAE=}. \
                    Skip pushing model to the model registry.')
